...
function isInsideFixedContainer(element: HTMLElement): boolean {
...
function onScrollToElement(
  ...
  // Elements inside fixed/sticky containers (e.g. sidebar) are always visible
...
function TourClose(props: TourCloseProps) {
  ...
  return (
    <ClosePrimitive
      ...
      className={cn(
        "absolute top-4 right-4 rtl:right-auto rtl:left-4 rounded-xs opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      onClick={onClick}
      {...closeButtonProps}
